...
*/
class PerchEvents_NLDisplayCalendar {
    ...
    private $timeslot_template = 'events/calendar/timeslot.html';

    private $booking_template = 'booking/booking.html';
    private $diary       = array();
    private $slots       = array();
    ...
    */
    public function set_day_name_format($f='01', $charlimit=0)
    {
    ...
    */
    public function display($templates=false)
    {
        ...
        if (is_array($templates)) {
            ...
            if (isset($templates['timeslot'])) $this->timeslot_template = $templates['timeslot'];
        }
        ...
        $this->EventSlotTemplate = $this->api->get('Template');
        $this->EventSlotTemplate->set($this->timeslot_template, 'events');
    ...
    public function display_day_names()
    {
        ...
        for($i=0; $i<7; $i++) {
          ...
          //  $dayname = strftime($this->day_name_format, strtotime($names[$i]));
             $dayname = date($this->day_name_format, strtotime($names[$i]));
    ...
    * @private
    */
    public function display_slots( $thisdate,$dow,$eventID,$matcheslots)
    {
...
//$matcheslottimes['availabletimes']=$availabletimes;
    ...
    */
    public function display_day_cell($day, $dow)
    {
        ...
        $thisdate = date("Y-m-d",$thisdate); //echo "thisdate->";
       // echo $thisdate;
        $output = false;     
        ...
        $event_days[]= $eventDate;
        }


           if (in_array($thisdate, $event_days)){

           //$Event->eventDateTime=$thisdate;
                $Event->squirrel('eventDateTime', $thisdate);
        ...
        if (count($matches)>0) {
            ...
            foreach($matches as $match){
               $str .= $this->EventTemplate->render($match);

               if($match->eventDateTime()>=date('Y-m-d')){
                  $matcheslottimes=  $this->display_slots( $match->eventDateTime(),$dow,$match->eventID(),$this->slots);

                               $html = $this->EventSlotTemplate->render_group($matcheslottimes);
                               $str .= $this->EventSlotTemplate->apply_runtime_post_processing($html);
               }
