...
class PerchEvents_Events extends PerchAPI_Factory
{
    ...
    public function all($Paging=false, $future=true)
    {
        ...
        if ($future) {
          ...
          $sql .= ' WHERE eventDateTime>='.$this->db->pdb(date('Y-m-d 00:00:00')).' OR eventEndDateTime>='.$this->db->pdb(date('Y-m-d 00:00:00'));

        }else{
            $sql .= ' WHERE eventEndDateTime<='.$this->db->pdb(date('Y-m-d 00:00:00'));
        }
    ...
    */
    public function find($eventID) {
		...
		if(is_array($row)) {
			...
			$row['cat_ids'] = $a;
			$sql = 'SELECT slotID FROM '.PERCH_DB_PREFIX.'events_to_timeslots WHERE eventID = '.$this->db->pdb($eventID);
            $result2 = $this->db->get_row($sql);
            if(is_array($result2)) {
            $row['slots'] = $result2["slotID"];
            }
...
/*
	...
	*/
    public function create_event_booking($data)
    {

      $this->db->insert(PERCH_DB_PREFIX.'events_bookings', $data);
    }
    /**
            	* takes the event data and inserts it as a new row in the database.
            	*/
                public function auto_create_events($slotid)
                {
                  $sql = " SELECT  COUNT(*) AS event_count, DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 1 MONTH), '%Y-%m-01') AS first_day_next_month,
                   DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 1 MONTH), '%M') AS next_month_name ,
                    LAST_DAY(DATE_ADD(CURDATE(), INTERVAL 1 MONTH)) AS next_month_last_date
                    FROM ".$this->table;

            $sql .= " WHERE eventDateTime >=  DATE_ADD(CURDATE(), INTERVAL 1 month)
          AND eventEndDateTime <= LAST_DAY(DATE_ADD(CURDATE(), INTERVAL 1 MONTH))";

        	$row = $this->db->get_row($sql);

        		if(is_array($row)) {
        		if($row["event_count"]==0){
        		$data['eventTitle']= $row["next_month_name"];
        	$data['eventDateTime'] = $row["first_day_next_month"];
            	$data['eventEndDateTime'] =  $row["next_month_last_date"];
        $data['slots']=$slotid;
	...
	*/
	...
	public function get_for_month($month,$year,$future_only=true,$cats=false) {
    	...
    	if (is_array($cats)) {
    	    $sql = 'SELECT DISTINCT e.*,e2t.*
    	            FROM '.$this->table.' e INNER JOIN '.PERCH_DB_PREFIX.'events_to_categories e2c ON   e.eventID=e2c.eventID INNER JOIN '.PERCH_DB_PREFIX.'events_categories c ON
    	            e2c.categoryID=c.categoryID LEFT JOIN '.PERCH_DB_PREFIX.'events_to_timeslots e2t ON  e.eventID=e2t.eventID
    	            WHERE  c.categorySlug IN ('.$this->implode_for_sql_in($cats).') ';
    	}else{
    	    $sql = 'SELECT DISTINCT e.*,e2t.* FROM '.$this->table.' e LEFT JOIN '.PERCH_DB_PREFIX.'events_to_timeslots e2t ON  e.eventID=e2t.eventID   WHERE 1=1  ';
    	}
        //$sql .= ' AND (eventDateTime>='.$this->db->pdb(date('Y-m-d 00:00:00')).' OR eventEndDateTime>='.$this->db->pdb(date('Y-m-d 00:00:00')).')';
    	...
    	/*if ($future_only) {
    	  ...
    	  $sql .= ' AND eventEndDateTime>=eventDateTime';
    	}        */
    ...
    public function get_timeslots_perdays($events) {
            ...
            if (PerchUtil::count($events)) {
                foreach($events as $Event) {
                    $out = $Event->to_array();
                      ...
                      if(isset($out["slotID"])){
                       $slotids []=  $out["slotID"];
                           $invalues=implode(",",$slotids);
                         $sql = 'SELECT * FROM '.PERCH_DB_PREFIX.'events_timeslots t, '.PERCH_DB_PREFIX.'events_timeslots_perday tpd where t.slotID=tpd.slotID AND tpd.enable=1 AND t.slotID='.$out["slotID"].' ';

                           $results    = $this->db->get_rows($sql);
                              $result[$out["eventID"]]=array();
                           if (PerchUtil::count($results)) {


                                foreach($results as $slotday) {

                            $result[$out["eventID"]][$slotday["dayID"]]=$slotday;
                            }
                        }
     ...
     public function get_display_slots( $thisdate,$dow,  $month='01', $year='1970',$opts=array())
     {
     // options
             $categories = false;
             $future_only = true;
             ...
             if (is_array($opts)) {
                 ...
                 if (isset($opts['category'])) {
                   ...
                   // echo "events";
                  //  print_r($events);
                     $slots = $this->get_timeslots_perdays($events);
                    //  echo "slots";
                                       //  print_r($slots);
                $DisplayTimeslots = new PerchEvents_TimeSlots($this->api);

            	$r = $DisplayTimeslots->display_event_slots($slots,$thisdate,$dow,$events);
    ...
    public function get_display($type='calendar', $month='01', $year='1970', $opts=array())
    {
        // options
        $categories = false;
        $future_only = true;
        ...
        switch($type) {
            case 'listing':
                ...
                if($slots){
                $DisplayListing->set_slots($slots);
                }
                ...
                break;
              case 'dropdown':
                             $DisplayDropDown = new PerchEvents_DisplayDropDown($this->api, $year, $month);
                             $DisplayDropDown->set_diary($events);
                             /*if($slots){
                             $DisplayDropDown->set_slots($slots);
                             }*/


                         	$r = $DisplayDropDown->display($templates);
                             break;
            default:
                $DisplayCalendar = new PerchEvents_DisplayCalendar($this->api, $year, $month);
                $DisplayCalendar->set_diary($events);
                if($slots){
                 $DisplayCalendar->set_slots($slots);
                }
    ...
    public function text_to_html($value)
    {
    	...
    	public static function register_member_login($Event)
    	{
    		...
    		$EventsRuntime->register_member_login($Event);
    	}
