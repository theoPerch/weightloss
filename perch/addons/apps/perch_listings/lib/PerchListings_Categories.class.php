...
class PerchListings_Categories extends PerchAPI_Factory
{
    protected $table     = 'listings_categories';
	protected $pk        = 'categoryID';
	protected $singular_classname = 'PerchListings_Category';
	...
	public function get_for_listing($listingID)
	{
	    $sql = 'SELECT c.*
	            FROM '.$this->table.' c, '.PERCH_DB_PREFIX.'listings_to_categories e2c
	            WHERE c.categoryID=e2c.categoryID
	                AND e2c.listingID='.$this->db->pdb($listingID);
	...
	public function get_custom($opts)
	{
        ...
        // post process       
        $tags   = $Template->find_all_tags_and_repeaters('listings');
	...
	public function update_listing_counts()
    {
    	$sql = 'SELECT c.categoryID, COUNT(p2c.listingID) AS qty
                FROM '.PERCH_DB_PREFIX.'listings_categories c, '.PERCH_DB_PREFIX.'listings_to_categories p2c, '.PERCH_DB_PREFIX.'listings_listings p
                WHERE p2c.categoryID=c.categoryID AND p2c.listingID=p.listingID
                    AND p.listingCreated>='.$this->db->pdb(date('Y-m-d H:i:00')).'
        ...
        if (PerchUtil::count($rows)) {
        	foreach($rows as $row) {
        		$sql = 'UPDATE '.PERCH_DB_PREFIX.'listings_categories SET categoryFutureEventCount='.$this->db->pdb($row['qty']).' WHERE categoryID='.$this->db->pdb($row['categoryID']).' LIMIT 1';
        		$this->db->execute($sql);
        	}
        }
        ...
        if (PerchUtil::count($rows)) {
        	foreach($rows as $row) {
        		$sql = 'UPDATE '.PERCH_DB_PREFIX.'listings_categories SET categoryEventCount='.$this->db->pdb($row['qty']).' WHERE categoryID='.$this->db->pdb($row['categoryID']).' LIMIT 1';
        		$this->db->execute($sql);
        	}
        }
    }
