...
class PerchMembers_Member extends PerchAPI_Base
{
    ...
    public function reset_password($send_notification_email=true)
    {
    	...
    	public function upload_member_file($SubmittedForm){
    //	echo "upload_member_file";print_r($SubmittedForm);
    	...
    	$Documents = new PerchMembers_Documents();
    	  if (isset($data['image'])){
    	        $Document = $Documents->upload($data['image'],$Session->get('memberID'),$fielddata['documentType']);

    	  }
    	    if (isset($data['video'])){

      $Document = $Documents->upload($data['video'],$Session->get('memberID'),$fielddata['documentType']);
      }
    ...
    //  perch_emailoctopus_update_contact($data);
      $this->sendtoadmin_docs_email($Session->get('memberID'),"george@nlclinicisleofwight.co.uk");
       $this->sendtoadmin_docs_email($Session->get('memberID'),"reshat1987@gmail.com");
    	}
