...
trait Output
{
    ...
    public function setEncodingFrom($str)
    {
        ...
        $str = filter_var($str, FILTER_SANITIZE_FULL_SPECIAL_CHARS, ['flags' => FILTER_FLAG_STRIP_LOW | FILTER_FLAG_STRIP_HIGH]);
        if (empty($str)) {
    ...
    {
        if (!is_null($filename)) {
            $filename = filter_var($filename, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_LOW);
