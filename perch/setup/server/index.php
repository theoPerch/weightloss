<?php
	...
	$fail = false;
	$results = array(); // Old style, as we've not tested PHP yet.
    ...
    if (version_compare(PHP_VERSION, '7.0.0', '<')) {
        ...
        $results[] = [
        				'status'   => 'failure',
        				'message' => "Perch requires PHP 7.0 or greater. You have PHP ".PHP_VERSION
        			];
    } else {
    	$results[] = [
        				'status'   => 'success',
    ...
    include(PERCH_CORE.'/templates/setup/top.php');
   	...
   	$body  = $HTML->wrap('p', "Your server does not meet the basic requirements to run this version of Perch at the moment. Please see below for the changes needed for a successful installation.");
