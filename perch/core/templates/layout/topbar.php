...
?>
    ...
    <nav class="utilnav">
        <ul>
<?php if (PERCH_RUNWAY) { ?>
        	<li><a href="<?php echo PerchUtil::html(PERCH_LOGINPATH); ?>/core/runway/search/" id="search-link"><?php echo PerchUI::png_icon('core/search'); ?> <span><?php echo PerchLang::get('Search'); ?></span></a></li>
<?php } ?>
<?php if ($CurrentUser->has_priv('perch.settings')) { ?>
		    <li><a href="<?php echo PerchUtil::html(PERCH_LOGINPATH); ?>/core/settings/" class="<?php if ($Perch->get_section()=='core/settings') echo 'selected'; ?>"><?php echo PerchUI::png_icon('core/gear'); ?> <span><?php echo PerchLang::get('Settings'); ?></span></a></li>
<?php } // settings ?>
...
<?php if ($Settings->get('helpURL')->val()) {
				echo '			<li><a href="'.PerchUtil::html($Settings->get('helpURL')->val()).'">'.PerchUI::png_icon('core/question').' <span>'.PerchLang::get('Help').'</span></a></li>';
            	}else{
				echo '			<li><a href="'.PERCH_LOGINPATH.'/core/help/">'.PerchUI::png_icon('core/question').' <span>'.PerchLang::get('Help').'</span></a></li>';
            	} 
            ?>
