...
class PerchTemplatedForm
{
		...
		if ($count > 0) {
			foreach($matches[0] as $match) {
			    ...
			    switch($Tag->type())
			    {
                    ...
                    case 'reCAPTCHA':
                        $template = $this->_replace_reCAPTCHA_field($match, $Tag, $template);
                        break;
    ...
    private function _replace_reCAPTCHA_field($match, $Tag, $template)
    {
        $attrs = $this->_copy_standard_attributes($Tag);

        $attrs['type'] = 'reCAPTCHA';

        $new_tag = ' <script src="https://www.google.com/recaptcha/api.js"></script>';

       $new_tag .= '<div class="g-recaptcha" data-sitekey="'.PerchUtil::html(PERCH_reCAPTCHA_SITE_KEY, true).'"></div>';
