...
class PerchUI
{
	...
	public static function png_icon($path, $size=16, $title=null, $custom_class=null)
	{
		$parts = explode('/', $path);
		$set  = $parts[0];
		$icon = $parts[1];
...
return '<img src="'.PERCH_LOGINPATH.'/core/assets/img/'.$icon.'.png"  width="'.$size.'" height="'.$size.'" class="'.$class.'"'.$title_atr.' />&nbsp;';
		//return '<img src="'.PERCH_LOGINPATH.'/core/assets/img/'.$icon.'.png" width="'.$size.'" height="'.$size.'" class="'.$class.'"'.$title_atr.'>';
	...
	{
		$t = strtotime($date);
		return date(PERCH_DATE_LONG.' '.PERCH_TIME_SHORT, $t);
	}
