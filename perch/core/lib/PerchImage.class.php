...
class PerchImage
{
        ...
        if (class_exists('SimpleXMLElement')) {
            ...
            if (is_object($XML)) {
                ...
                if ($out['w'] == 0) {
                    ...
                    if ($viewbox) {
                        $out = $this->getSvgDimensionsFromViewbox($viewbox);
                    }
                }

                // In case the SVG has no dimensions
                if ($out['w'] == 0) {
                    $out['w'] = 150;
                    $out['h'] = 150;
                }
    ...
    private function resize_with_imagick($image_path, $save_as, $new_w=0, $new_h=0, $crop_w=0, $crop_h=0, $crop_x=0, $crop_y=0)
    {    
        $crop    = false;
        if ($crop_w != 0 && $crop_h != 0) $crop = true;
    ...
    private function getSvgDimensionsFromViewbox($viewbox)
    {
        $out = [];
        $out['w'] = 0;
        $out['h'] = 0;

        if (strpos($viewbox, ',') !== false) {
            $dims = explode(',', $viewbox);
        } else {
            $dims = explode(' ', $viewbox);
        }

        if (PerchUtil::count($dims)) {
            $out['w'] = (int)trim($dims[2]);
            $out['h'] = (int)trim($dims[3]);
        }
