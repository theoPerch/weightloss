...
class PerchFieldType_date extends PerchFieldType
{
    ...
    public function get_search_text($raw=false)
    {
        ...
        return date('l d M Y', strtotime($raw));
    }
...
class PerchFieldType_chat extends PerchFieldType
{
    ...
    public function render_inputs($details=array())
    {
        if (!defined('PERCH_CHAT_PAGE_ID')) {
            $API = new PerchAPI(1.0, $this->app_id);
            $HTML = $API->get('HTML');
            return $HTML->warning_message(PerchLang::get('To create a FB chat you must set up a Messnger PAGE ID.'));
        }

        $s = $this->Form->text($this->Tag->input_id().'_chat_version', $this->Form->get((isset($details[$this->Tag->input_id()])? $details[$this->Tag->input_id()] : array()), 'chat_version', $this->Tag->default()), ' input-simple m');
    ...
    public function get_raw($post=false, $Item=false)
    {
        ...
        $chat_version='v14.0';
         if (isset($post[$this->Tag->id().'_chat_version']) && $post[$this->Tag->id().'_chat_version']!='') {
          $chat_version=$post[$this->Tag->id().'_chat_version'];
         }


          $var = $this->_process_chat($chat_version);
    ...
    private function _get_page_id()
    {
        ...
        return PERCH_CHAT_PAGE_ID;
    }

    private function _process_chat($chat_version)
    {
        $out = array();

           if (isset($chat_version)) {

                    $out['chat_version']     = $chat_version;

            $r  = '<div id="fb-root"></div><div id="fb-customer-chat" class="fb-customerchat"></div>';

            //$out['admin_html'] = $r;

            // JavaScript
            $r .= '<script > ';
              $r .= " var chatbox = document.getElementById('fb-customer-chat');";
              $r .= 'chatbox.setAttribute("page_id", "'.PerchUtil::html($this->_get_page_id(), true).'");';
             $r .= ' chatbox.setAttribute("attribution", "biz_inbox");';
              $r .= '</script>';
             $r .= '<script > ';
            $r .= "  window.fbAsyncInit = function() {
                       FB.init({
                         xfbml            : true,
                         version          : 'v14.0'
                       });
    ...
    public function get_index($raw=false)
    {
        ...
        if (is_array($raw)) {
           ...
           $rawvalue=$raw;
            ...
            $out[] = array('key'=>$id, 'value'=>$rawvalue);
        }
