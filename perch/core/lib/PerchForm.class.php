...
class PerchForm
{
    ...
    public function datetimepicker($id, $value=false, $field_order='dmy', $allowempty=false)
    {
        ...
        //8.1
        $value  = ($this->value($value) ? $this->value($value) :  date('Y-m-d H:i'));

        $d          = [];
        $d['day']   = date('d', strtotime($value)); //strftime('%d', strtotime($value));
        $d['month'] = date('m', strtotime($value));  //strftime('%m', strtotime($value));
        $d['year']  = date('Y', strtotime($value));  //strftime('%Y', strtotime($value));
        $d['hour']  =  date('H', strtotime($value)); //strftime('%H', strtotime($value));
        $d['minute']= date('i', strtotime($value)); //strftime('%M', strtotime($value));
    ...
    public function timepicker($id, $value=false)
    {
        ...
        $value  = ($this->value($value) ? $this->value($value) : date('H:M'));
	...
	public function radio_set($id,$group_set, $label, $options, $value, $class='', $limit=false, $container_class=false)
    {
        ...
        foreach($options as $option) {
            ...
            $out .= '<div class="radio-single">';
            $out .= $this->radio($boxid,$group_set, $option['value'], $checked_value, (isset($option['class']) ? $option['class'] : false),$option['disabled']);
            $out .= '<div class="form-entry">';
            $out .= $this->label($boxid, $option['label'], '', $colon=false, $translate=false);
            $out .= '</div>';
            $out .= '</div>';
	...
	public function submit($id, $value, $class=false, $translate=true, $use_button=false, $icon=false)
	{
		...
		public function multi_image($id, $value='', $basePath='', $class='')
        	{
        		if ($this->display_only) {
        		...
        		$s	= '<input type="file" multiple id="'.$this->html($id, true).'" name="'.$this->html($id, true).'" value="'.$this->html($this->value($value), true).'" class="'.$this->html($class, true).'" title="'.$this->html($this->value($value), true).'" />';
