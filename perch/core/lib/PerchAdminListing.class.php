	...
	private function template_data_cell($row, $col)
	{
		...
		if (isset($col['edit_link']) && $col['edit_link']) {
			...
			if ($link) {
				...
				$s .= '<a id="'. $row->id().'" class="region '.($icon_used ? '' : $class).'" href="'.$this->HTML->encode($link).'">';
				if ($icon_used) {
					$s .= $icon_value . $this->HTML->wrap('span.'.$class, $cell_value);
				} else {
					$s .= $cell_value;	
				}
				$s .= '</a><span style="display: none;    padding: 10px; border: 1px solid #ccc; background-color: white; border-radius: 5px;" id="tooltip'.$row->id().'">'. $row->regionHTML().'</span>';
	...
	private function format_value($def, $value)
	{
		...
		switch($def['format']['type']) {
			case 'date':
				...
				$value = date($def['format']['format'], strtotime($value));
				break;
		}
