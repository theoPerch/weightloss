			...
			$Listing->add_col([
			    ...
			    'value'     => function($item, $HTML) {
			    	if ($item->regionUpdated()!='0000-00-00 00:00:00') {
						return date(PERCH_DATE_SHORT .' '.PERCH_TIME_SHORT, strtotime($item->regionUpdated()));
					}else{
						return '&dash;';
					}
			    },
		...
		$CollectionListing->add_col([
		    ...
		    'value'     => function($item, $HTML) {
		    	if ($item->collectionUpdated()!='0000-00-00 00:00:00') {
					return date(PERCH_DATE_SHORT .' '.PERCH_TIME_SHORT, strtotime($item->collectionUpdated()));
				}else{
					return '&dash;';
				}
		    },
