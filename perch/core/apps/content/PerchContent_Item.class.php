...
class PerchContent_Item extends PerchBase
{
	...
	public function log_user_actions()
	{
		...
		$sql    = 'INSERT IGNORE INTO '.PERCH_DB_PREFIX.'user_log(`appID`, `itemFK`, `itemRowID`, `userID`,`logTime`) VALUES';
        $sql    .= '('."'".$this->app_id."','itemRowID',".(int)$this->itemRowID().','.(int)$CurrentUser->id().',"'.date('Y-m-d H:i:s').'")';
        $this->db->execute($sql);

       $Perch = Perch::fetch();
       $Perch->event('item.log_user_actions', $this);
	}
